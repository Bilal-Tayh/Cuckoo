...
// 	V _values[hh_nr_bins][8] = {};
    ...
    void get8samples();
//     void get8samples1();
	void move_to_bin(K key, V value, int bin,  V kDelta);
...
Cuckoo_waterLevel_HH_no_FP_SIMD_256<K, V>::Cuckoo_waterLevel_HH_no_FP_SIMD_256(int seed1, int seed2, int seed3, float max_load, float gamma, int n) {
	...
	_max_load = hh_nr_bins * 4 * max_load;
	_insertions_till_maintenance_restart = _max_load;
...
// 		_values[bin1][empty_spot_in_bin1] = _water_level+value;
        ...
        Delta[bin1][empty_spot_in_bin1] = _water_level;
...
// 		_values[bin2][empty_spot_in_bin2] = _water_level + value;
        ...
        Delta[bin2][empty_spot_in_bin2] = _water_level;
//         print();
		return;
	}
	K kicked_key;
	V kicked_value;
    uint64_t kicked_delta;
...
//     print();
    ...
    if (kicked_bin_1 != kicked_from_bin) {
		move_to_bin(kicked_key, kicked_value, kicked_bin_1,kicked_delta);
	}
	else {
		uint64_t kicked_bin_2 = (kicked_hash >> 32) & _mask;
		move_to_bin(kicked_key, kicked_value, kicked_bin_2,kicked_delta);
	}
...
template<class K, class V>
void Cuckoo_waterLevel_HH_no_FP_SIMD_256<K, V>::move_to_bin(K key, V value, int bin, V kDelta)
{
...
// 		_values[bin][empty_spot_in_bin] = value;
        ...
        Delta[bin][empty_spot_in_bin]= kDelta;
//         counter = 0;
		return;
	}
...
//         return _keys[bin1][4+(tz1 >> 2)];
        return _keys[bin1][4+(tz1 >> 2)] - Delta[bin1][(tz1 >> 2)];
	}
	uint64_t bin2 = (hash >> 32) & _mask;
...
// 		return _keys[bin2][4+(tz2 >> 2)];
        return _keys[bin2][4+(tz2 >> 2)] - Delta[bin2][(tz2 >> 2)];
	}
	return _water_level;
// return 0;
}
